...
const Stats = () => {
  const { statsData, updateStatsData } = useApp();
  ...
  const updateCharts = () => {
    ...
    updatedStats.orders.datasets[0].data = updatedStats.orders.datasets[0].data.map(
      value => randomUpdate(value, 15)
    );
    
    // Mettre à jour le graphique de livraison
    updatedStats.delivery.datasets[0].data = [
      randomUpdate(33, 3),
      randomUpdate(52, 3),
      randomUpdate(15, 2)
    ];
    ...
    updatedStats.products.datasets[0].data = updatedStats.products.datasets[0].data.map(
      value => randomUpdate(value, 3)
    );
    
    // Mettre à jour la satisfaction
    updatedStats.satisfaction.datasets[0].data = updatedStats.satisfaction.datasets[0].data.map(
      value => randomUpdate(value, 1)
    );
    
    // Mettre à jour les revenus
    updatedStats.revenue.datasets[0].data = updatedStats.revenue.datasets[0].data.map(
      value => randomUpdate(value, 10)
    );
    
    // Mettre à jour les heures d'affluence
    updatedStats.hours.datasets[0].data = updatedStats.hours.datasets[0].data.map(
