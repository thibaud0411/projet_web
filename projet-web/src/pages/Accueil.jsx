...
const Accueil = () => {
  ...
  useEffect(() => {
    ...
    const liveData = {
      orders: orders.filter(order => order.date === new Date().toISOString().split('T')[0]).length,
      customers: [...new Set(orders.map(order => order.customer))].length,
